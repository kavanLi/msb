...
public class CommitLog implements Swappable {
    ...
    public CommitLog(final DefaultMessageStore messageStore) {
        ...
        //MappedFileQueue 这个就是准备和具体的文件队列的一个队列
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessage(final MessageExtBrokerInner msg) {
        ...
        try {
            ...
            //这里是一把锁（写消息的锁：这里是为了确保写入 commitlog不会发生并发安全问题）：
            ...
            try {
                ...
                //这里就是进行消息的追加（按照顺序的方式写入mappedFile 中）
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                try {
                    ...
                    long begin = System.currentTimeMillis();
                    CommitLog.this.mappedFileQueue.flush(flushPhysicQueueLeastPages); //这里是刷盘
    ...
    /**
     * GroupCommit Service  本质是runnable
     */
        ...
        private void doCommit() {
            if (!this.requestsRead.isEmpty()) {
                for (GroupCommitRequest req : this.requestsRead) {
                    ...
                    for (int i = 0; i < 2 && !flushOK; i++) { //这里flush2次
                        CommitLog.this.mappedFileQueue.flush(0);  //这里就是刷盘
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // Write messages to the queue buffer  ---这里是进行消息的写入（仅仅是对MMAP的内存映射进行put  没有到磁盘）
    ...
    class DefaultFlushManager implements FlushManager {
        ...
        public DefaultFlushManager() {
            ...
            if (FlushDiskType.SYNC_FLUSH == CommitLog.this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
                this.flushCommitLogService = new CommitLog.GroupCommitService();//同步刷盘
            } else {
                this.flushCommitLogService = new CommitLog.FlushRealTimeService();//异步刷盘
            }
        ...
        @Override public void start() {
            ...
            if (defaultMessageStore.isTransientStorePoolEnable()) { //这里直接内存刷盘的线程
            }
        ...
        public void handleDiskFlush(AppendMessageResult result, PutMessageResult putMessageResult,
            ...
            if (FlushDiskType.SYNC_FLUSH == CommitLog.this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
                ...
                if (messageExt.isWaitStoreMsgOK()) {
                    ...
                    service.putRequest(request); //这里进行触发
        ...
        public CompletableFuture<PutMessageStatus> handleDiskFlush(AppendMessageResult result, MessageExt messageExt) {
            ...
            if (FlushDiskType.SYNC_FLUSH == CommitLog.this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
                final GroupCommitService service = (GroupCommitService) this.flushCommitLogService;
                if (messageExt.isWaitStoreMsgOK()) {//这里是进行主从同步
                    GroupCommitRequest request = new GroupCommitRequest(result.getWroteOffset() + result.getWroteBytes(), CommitLog.this.defaultMessageStore.getMessageStoreConfig().getSyncFlushTimeout());
