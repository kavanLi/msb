...
public class DefaultHAClient extends ServiceThread implements HAClient {
    ...
    private ByteBuffer byteBufferRead = ByteBuffer.allocate(READ_MAX_BUFFER_SIZE);//read的缓冲区
    ...
    private void reallocateByteBuffer() {
        ...
        if (remain > 0) { // 如果byteBufferRead中还有未处理的字节的时候
        ...
        this.dispatchPosition = 0;  // 从0开始处理byteBufferRead中的数据
    }
    ...
    private boolean dispatchReadRequest() {
        // 定义消息头大小为8+4字节，即物理偏移量和消息体大小
        final int msgHeaderSize = 8 + 4; // phyoffset + size
        int readSocketPos = this.byteBufferRead.position();    // 获取当前读取位置

        while (true) {
            // 计算当前读取位置与待处理位置的差值
            int diff = this.byteBufferRead.position() - this.dispatchPosition;
            if (diff >= msgHeaderSize) {  // 如果差值大于等于消息头大小
                // 读取物理偏移量和消息体大小
                long masterPhyOffset = this.byteBufferRead.getLong(this.dispatchPosition);
                int bodySize = this.byteBufferRead.getInt(this.dispatchPosition + 8);
                // 获取从节点的最大物理偏移量
                long slavePhyOffset = this.defaultMessageStore.getMaxPhyOffset();
                // 如果从节点的最大物理偏移量不为0且不等于主节点的物理偏移量，则返回false
                if (slavePhyOffset != 0) {
            ...
            if (!this.byteBufferRead.hasRemaining()) {
                //重新分配缓冲区的方法。
                //在读取请求转发过程中，如果缓冲区中没有足够的数据来读取消息，则需要重新分配一个更大的缓冲区，以便可以继续读取消息
    ...
    @Override
    public void run() {  //遇到serviceThread 直接看run方法
        ...
        while (!this.isStopped()) {
            try {
                switch (this.currentState) {
                    case SHUTDOWN:
                        return;
                    case READY: //第一步（建立与主的连接，注册read事件）
                        if (!this.connectMaster()) { //这里是连接master
                            log.warn("HAClient connect to master {} failed", this.masterHaAddress.get());
                            this.waitForRunning(1000 * 5);
                        }
                        continue;
                    case TRANSFER://第二步（进行数据传输，响应read事件）
