...
public class ConsumeMessageConcurrentlyService implements ConsumeMessageService {
    ...
    public void processConsumeResult(
        ...
        switch (status) {
            case CONSUME_SUCCESS:            // 消费成功
                ...
                break;
            case RECONSUME_LATER:// 消费失败，ackIndex = -1
                ackIndex = -1;
        ...
        switch (this.defaultMQPushConsumer.getMessageModel()) {
            ...
            case CLUSTERING:
                ...
                for (int i = ackIndex + 1; i < consumeRequest.getMsgs().size(); i++) {
                    ...
                    boolean result = this.sendMessageBack(msg, context);// 向broker发起消息回退请求
                    if (!result) {
        ...
        // 把被真正消费成功的消息从msgTreeMap中移除，怎样才算真正的消费成功？
        // 这里消费成功的 或者消费失败但是消息回退成功都算是真正的消费成功
    ...
    public boolean sendMessageBack(final MessageExt msg, final ConsumeConcurrentlyContext context) {
        int delayLevel = context.getDelayLevelWhenNextConsume(); // 发送回退消息的延迟级别（因为发送回退消息也是发送延迟消息的一种）
