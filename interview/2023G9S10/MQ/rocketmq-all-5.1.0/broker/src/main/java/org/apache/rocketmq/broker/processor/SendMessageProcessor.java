...
//这里是Broker接收消息的入口（生产者发送的消息到了Broker就会进入这里）
public class SendMessageProcessor extends AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx,
        ...
        switch (request.getCode()) {
            case RequestCode.CONSUMER_SEND_MSG_BACK:  //如果是消费者 回退消息（消费失败，作为生产者 发回退请求）
    ...
    public RemotingCommand sendMessage(final ChannelHandlerContext ctx,
        ...
        //包括队列ID（客户端生产者是可以在方式的时候 指定 这个消息的队列ID）
        ...
        //MessageExtBrokerInner 是在Broker中消息进一步封装
        ...
        if (brokerController.getBrokerConfig().isAsyncSendEnable()) {
            ...
            //这里是处理消息的核心
            if (sendTransactionPrepareMessage) { //这里是事务消息
