...
public abstract class AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    protected RemotingCommand consumerSendMsgBack(final ChannelHandlerContext ctx, final RemotingCommand request)
        ...
        // 根据消费者组名找到对应的SubscriptionGroupConfig对象
        ...
        // 如果重试队列数 <= 0, 那么直接返回SUCCESS
        if (subscriptionGroupConfig.getRetryQueueNums() <= 0) {
            response.setCode(ResponseCode.SUCCESS);
            response.setRemark(null);
            return response;
        }
        //这里要往重试主题中发消息（"%RETRY%"+group）
        String newTopic = MixAll.getRetryTopic(requestHeader.getGroup());
        // 得到的queueIdInt通常都等于0，也就是说对于每一个消费者组重试队列只有一个
        ...
        // 根据commitlog物理偏移量从commitlog文件中返回对应的消息对象
        ...
        if (null == retryTopic) {
            // 把消息的主题设置到属性RETRY_TOPIC中
            MessageAccessor.putProperty(msgExt, MessageConst.PROPERTY_RETRY_TOPIC, msgExt.getTopic());
        }
        msgExt.setWaitStoreMsgOK(false);
        // 得到延迟级别
        int delayLevel = requestHeader.getDelayLevel();
        // 获取最大消息重试次数(新版本都是客户端传过来的最大消息重试次数)
        ...
        //死信队列的处理
        //该消息回退次数已经到达了客户端指定的重试次数，或者延迟级别 < 0, 消息就会进入死信队列
        boolean isDLQ = false;
        if (msgExt.getReconsumeTimes() >= maxReconsumeTimes
            ...
            //如果用户的业务逻辑中没有对ConsumeConcurrentlyContext对象的delayLevelWhenNextConsume属性进行设置
            // delayLevel == 0
            if (0 == delayLevel) {  //第一次进行重试（3的延时等级 10s）
        ...
        // 设置死信主题 / 重试主题
        msgInner.setTopic(newTopic);//（"%RETRY%"+group）
