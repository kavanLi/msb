...
public class NamesrvStartup {
    ...
    public static NamesrvController main0(String[] args) {
        try {
            parseCommandlineAndConfigFile(args);//解析命令行参数与配置文件->***Config类
            NamesrvController controller = createAndStartNamesrvController();//NamesrvController的初始化与启动
            return controller;
        } catch (Throwable e) {
            e.printStackTrace();
            System.exit(-1);
    ...
    public static NamesrvController createNamesrvController() {
        ...
        controller.getConfiguration().registerConfig(properties);//nameServer自身的配置
        return controller;
    }
