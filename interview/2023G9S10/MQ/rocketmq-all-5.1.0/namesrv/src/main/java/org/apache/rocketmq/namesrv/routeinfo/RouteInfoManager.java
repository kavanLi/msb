...
public class RouteInfoManager {
    ...
    private final Map<String/* topic */, Map<String, QueueData>> topicQueueTable;//主题队列信息
    private final Map<String/* brokerName */, BrokerData> brokerAddrTable;//broker信息
    private final Map<String/* clusterName */, Set<String/* brokerName */>> clusterAddrTable;//集群信息
    private final Map<BrokerAddrInfo/* brokerAddr */, BrokerLiveInfo> brokerLiveTable;//broker存活信息
    private final Map<BrokerAddrInfo/* brokerAddr */, List<String>/* Filter Server */> filterServerTable;//过滤服务器信息
    private final Map<String/* topic */, Map<String/*brokerName*/, TopicQueueMappingInfo>> topicQueueMappingInfoTable;//broker和主题队列映射信息

    private final BatchUnregistrationService unRegisterService;//批量取消注册Broker的线程
    ...
    public TopicList getAllTopicList() {
        ...
        try {
            this.lock.readLock().lockInterruptibly();  //这里是一把读锁
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            //利用写锁来确保同一时间只有一个线程可以执行
            this.lock.writeLock().lockInterruptibly();

            //init or update the cluster info  集群消息
...
class BrokerLiveInfo {
    private long lastUpdateTimestamp;  //这台Broker主机最近的一次心跳信息
    private long heartbeatTimeoutMillis; //这里是 超时的阈值（如果超过这个时间，就认为这个broker不可用了） 默认是1秒钟
