...
import com.mashibing.internal.common.config.easyExcel.handler.DropdownWriteHandler;
import com.mashibing.internal.common.config.easyExcel.handler.MultiWaterMarkHandler;
import com.mashibing.internal.common.config.easyExcel.handler.SingleWaterMarkHandler;
...
public class EasyExcelUtils {
    ...
    public static String DEFAULT_WATER_MARK_CONTENT = "allinpay";
    public static String DAILY_FEE_FLOW_FILE_NAME = "出入金日结手续费账单-";
    public static String ITEM_NAME = "款项信息-";
    public static String ITEM_IMPORT_TEMPLATE = "款项批导模板";
     ...
     * @throws IOException
     */
    public static void excel4MultiProtectedTablesDownload(HttpServletResponse response, String fileName,
                                                          List <SheetTableData> sheetTableDataList, String waterMarkContent, Boolean protectFlag) throws IOException {
        ...
        try {
            ...
            ExcelWriter writer = EasyExcel.write(outputStream)
                    ...
                    .registerWriteHandler(new DropdownWriteHandler(sheetTableDataList)) // 设置下拉框
                    .build();
            ...
            // 设置居中对齐
            setCellAlignment2Center(workbook);
     ...
     * @param workbook
     */
    private static void setCellAlignment2Center(Workbook workbook) {
        XSSFSheet sheet = (XSSFSheet) workbook.getSheetAt(0);
        ...
        for (Row row : sheet) {
            for (Cell cell : row) {
                if (cell.getCellType() == CellType.NUMERIC && DateUtil.isCellDateFormatted(cell)) {
                    cell.setCellStyle(dateStyle);  // 日期单元格，应用日期格式
                } else {
                    cell.setCellStyle(centerStyle);  // 非日期单元格，应用居中对齐
                }
            }
        }
    }
