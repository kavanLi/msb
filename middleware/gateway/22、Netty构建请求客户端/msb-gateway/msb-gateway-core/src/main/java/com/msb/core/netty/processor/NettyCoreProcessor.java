...
public class NettyCoreProcessor implements NettyProcessor{
    @Override
    public void process(HttpReqeustWrapper httpReqeustWrapper) {
        FullHttpRequest request = httpReqeustWrapper.getRequest();
        ChannelHandlerContext ctx = httpReqeustWrapper.getCtx();
        try{
            // 1、创建网关的上下文
            ...
            log.error("process unnkown error",t);
    ...
    private void complete(Request request, Response response,
                          Throwable throwable, GatewayContext gatewayContext) {
        // 释放资源
        ...
        try{
            if(Objects.nonNull(throwable)){
                ...
                if(throwable instanceof TimeoutException){
                    ...
                    gatewayContext.setThrowalbe(new ResponseException(ResponseCode.REQUEST_TIMEOUT));
                }else{
                    // 其他异常
                    gatewayContext.setThrowalbe(new ConnectException(throwable, gatewayContext.getUniqueId(),
            ...
            gatewayContext.setThrowalbe(new ResponseException(ResponseCode.INTERNAL_ERROR));
